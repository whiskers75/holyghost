...
Jesus.prototype.start = function start(args, options) {
    ...
    if (typeof options.id !== "string") {
        options.id = [0, 0, 0, 0].map(function (e) {
    ...
    if (!this.processes[options.id]) {
        var onExit = function onExit(code) {
            ...
            setTimeout(function () {
                ...
                this.processes[options.id].out = this.openLog(options.id, "out");
                this.processes[options.id].err = this.openLog(options.id, "err");
                this.processes[options.id].out.write('holyghost: detected script exit, restarting\n');
    ...
    this.processes[options.id].out = this.openLog(options.id, "out");
    this.processes[options.id].err = this.openLog(options.id, "err");
    child.stdout.pipe(this.processes[options.id].out);
    child.stderr.pipe(this.processes[options.id].err);
...
Jesus.prototype.restartAll = function restartAll() {
    Object.keys(this.processes).map(function (k) {
